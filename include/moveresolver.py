...
class MoveResolver:
    ...
    def resolve(self, annotation, white):
        ...
        if "O-O-O" in annotation:
            ...
            if not annotation[:1].isupper():  # pawn movement
                ...
                if not ("x" in annotation):
                    ...
                    for i in range(1, 3):
                        new_annotation = annotation[:1] + str(int(annotation[1:]) + i * direction)
                        if not self.board.checkPosition(new_annotation) == " ":
                ...
                for piece in (self.board.whitePieces if white else self.board.blackPieces):
                    if piece[0] == p or piece[0].upper() == p and (frm == "" or frm in piece[1]):
                        (new_annotation, options) = self.pc.checkMovesPiece(piece[1])
